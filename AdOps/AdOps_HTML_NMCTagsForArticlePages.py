...
#  This script is checking Article Pages
#
# Acceptance Criteria:
# 1) nmc:pid should be content=”article_<headline>"
# 2) nmc:tag should be content=”article, <sectionfront name>, <sectionfront secondary_name>”
#
# -------------------------------------------------------------------------------------------------
...
with open(LIBRARY_PATH + "/articleURLs.json") as file:
    article_urls = json.load(file)

# Run the test with the URLs - get the number of pages failed.
pages_failed = NMCTest.singleTest(article_urls)
